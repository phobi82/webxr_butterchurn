...
### Changed
- Replaced the triangle-grid forward reprojection in `xr-depth.js` with a fullscreen inverse reprojection: for each render pixel the shader looks up the corresponding sensor UV, decodes raw depth inline, and reconstructs the exact 3D point. Hardware bilinear upscaling (LINEAR texture filter) handles the 320×320 → render-resolution step for free. This removes all grid buffers, corner-validity checks, and the `Grid Factor` menu control, and produces correct depth masks with no triangle-edge artifacts.
