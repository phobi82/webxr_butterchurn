...
### Changed
- Replaced the triangle-grid forward reprojection in `xr-depth.js` with a texture-based inverse reprojection path that canonicalizes source depth once and renders one target-space depth texture directly in sensor resolution instead of full render resolution. `xr-depth` now owns the upscale behavior centrally, so consumers keep the same shared 2D depth-texture contract without reprojecting or upscaling on their own.
