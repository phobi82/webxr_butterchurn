...
- Changed passthrough punch, world masking, and depth-aware modified-reality lighting to consume the shared final `depth + worldPoint` texture directly, removing per-consumer world-point reconstruction, legacy depth UV transforms, and the old split between array-texture and plain-texture canonicalization programs.
- Replaced the triangle-grid forward reprojection in `xr-depth.js` with a texture-based inverse reprojection path that canonicalizes source depth once, reprojects it once per view in sensor resolution, and then centrally builds one shared high-resolution consumer depth texture instead of letting each consumer reproject or upscale on its own.
- Rebuilt `xr-depth.js` as the single depth adapter and reprojection stage: CPU, `gpu-texture`, and `gpu-array` sources are normalized once, reprojected once per view, optionally converted from low-resolution target-planar depth to target-radial depth, and finally upscaled once into the shared high-resolution texture that passthrough punch, world masking, and depth-aware lighting reuse.
