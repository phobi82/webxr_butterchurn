...
- **Inverse Reprojection**: `xr-depth` canonicalizes runtime depth once per eye, then runs a fullscreen inverse reprojection — for each render pixel the shader looks up the sensor UV, decodes raw depth inline, and hardware bilinear upscaling (LINEAR filter) handles the 320×320 → render-resolution step for free, producing one target-space 2D depth texture that passthrough punch, world masking, and depth-aware lighting all reuse
