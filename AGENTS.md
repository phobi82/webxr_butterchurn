...
- For Pico Browser remote debugging, inspect `/proc/net/unix` for `weblayer_devtools_remote_<pid>`, forward `tcp:9222` to that exact localabstract socket, and refresh `http://127.0.0.1:9222/json/list` before attaching because the socket PID can change after browser restarts.
- For Pico WebXR depth debugging, verify depth in the manually entered live XR session with temporary injection around `XRSession.requestAnimationFrame` or `XRFrame.getViewerPose`; do not infer depth support from exposed constructors alone.
- If testing Pico Browser WebXR flags, record the exact flag values, force-stop and restart `com.pico.browser.overseas`, retest in a manually entered session, and restore the previous flag values after the test unless the user explicitly wants to keep them.
